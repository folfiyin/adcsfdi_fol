...
for chunk in df2:
    ...
    def rMS(chunk): #calculate root mean square
        rms = np.sqrt(np.mean(chunk**2))
        return rms
    RMS.append(rMS(chunk))
    
    def crest_factor(chunk): #calculate crest factor
        return np.max(np.abs(chunk))/np.sqrt(np.mean(np.square(chunk)))
    Crst.append(crest_factor(chunk))
